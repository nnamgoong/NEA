...
class SubtractiveSynth:
    def __init__(self, parent, sample_rate, duration, update_presets_callback, user_id, preset_manager):
        ...
        # Initialize components with callback support
        self.oscillator = Oscillator(sample_rate, on_change_callback=self.update_graphs)
        self.filter = Filter(sample_rate, on_change_callback=self.update_graphs)
        self.effect = Effect(sample_rate, on_change_callback=self.update_graphs)
        self.lfo = LFO(sample_rate, on_change_callback=self.update_graphs)

        # Parameters
        self.volume = 0.5
        self.update_timer = None  # Initialize the update timer

        # Track preset name
        self.loaded_preset_name = None  # Initialize the loaded preset name
    ...
    def create_ui(self):
        ...
        self.controls_frame = ctk.CTkFrame(self.parent, width=700)
        ...
        self.add_oscillator_button = ctk.CTkButton(
            ...
            text="Add Oscillator", 
            command=lambda: self.oscillator.add_oscillator(self.oscillators_frame, osc_type="Sine", frequency=440.0, amplitude=0.5)
        )
        ...
        self.add_filter_button = ctk.CTkButton(
            self.scrollable_frame, 
            text="Add Filter", 
            command=lambda: self.filter.add_filter(self.filters_frame, filter_type="Low-pass", cutoff=1000.0, resonance=1.0)
        )
        ...
        self.add_effect_button = ctk.CTkButton(
            self.scrollable_frame, 
            text="Add Effect", 
            command=lambda: self.effect.add_effect(self.effects_frame, effect_type="Bitcrusher", params=None)
        )
        ...
        self.add_lfo_button = ctk.CTkButton(
            self.scrollable_frame, 
            text="Add LFO", 
            command=lambda: self.lfo.add_lfo(self.lfos_frame, shape="Sine", frequency=1.0, depth=0.5, target="Frequency")
        )
    ...
    def update_graphs(self):
        ...
        try:
            max_freq = max(
                [float(osc["frequency"].get()) for osc in self.oscillator.oscillators if osc["frequency"].get().strip()]
                + [1]
            )
        except ValueError:
    ...
    def update_graphs(self):
        """Regenerate and redraw the waveform and filter graphs."""
        waveform = self.generate_waveform()
        filtered_waveform = self.filter.apply_filters(waveform)
    ...
    def generate_waveform(self):
        ...
        for osc in self.oscillator.oscillators:
            ...
            freq_modulation = self.lfo.apply_lfo("Frequency", t)
            ...
            amp_modulation = self.lfo.apply_lfo("Amplitude", t)
    ...
    def load_preset(self, preset_data):
        ...
        for osc in self.oscillator.oscillators:
            osc["frame"].destroy()
        self.oscillator.oscillators.clear()

        for osc in preset_data.get("oscillators", []):
            self.oscillator.add_oscillator(
                self.oscillators_frame,  # Pass the oscillators_frame
        ...
        for f in preset_data.get("filters", []):
            self.filter.add_filter(
                self.filters_frame,  # Pass the filters_frame
                f["type"],  # Filter type
        ...
        self.effect.effects.clear()

        for e in preset_data.get("effects", []):
            self.effect.add_effect(
                self.effects_frame,  # Pass the effects_frame
                e["type"],  # Effect type
                e["params"]  # Effect parameters
            )
        ...
        for l in preset_data.get("lfos", []):
            self.lfo.add_lfo(
                self.lfos_frame,  # Pass the lfos_frame
                l["shape"],  # LFO shape
                float(l["frequency"]),  # LFO frequency
                float(l["depth"]),  # LFO depth
                l["target"]  # LFO target
            )
...
class Oscillator:
    def __init__(self, sample_rate, on_change_callback=None):
        self.sample_rate = sample_rate
        self.oscillators = []  # List to store active oscillators
        self.on_change_callback = on_change_callback

    def add_oscillator(self, oscillators_frame, osc_type="Sine", frequency=440.0, amplitude=0.5):
        """Add a new oscillator to the oscillators chain."""
        osc_frame = ctk.CTkFrame(oscillators_frame)
        ...
        amp_slider = ctk.CTkSlider(osc_frame, from_=0.0, to=1.0, command=lambda _: self.notify_change())
    ...
    def remove_oscillator(self, osc_frame):
        """Remove an oscillator from the oscillators chain."""
    ...
    def notify_change(self):
        """Notify the parent class (SubtractiveSynth) that a change has occurred."""
...
class Filter:
    def __init__(self, sample_rate, on_change_callback=None):
        self.sample_rate = sample_rate
        self.filters = []  # List to store active filters
        self.on_change_callback = on_change_callback

    def add_filter(self, filters_frame, filter_type="Low-pass", cutoff=1000.0, resonance=1.0):
        """Add a new filter to the filters chain."""
        filter_frame = ctk.CTkFrame(filters_frame)
        ...
        freq_slider = ctk.CTkSlider(filter_frame, from_=20, to=10000, command=lambda _: self.notify_change())
        ...
        res_slider = ctk.CTkSlider(filter_frame, from_=0.1, to=10, command=lambda _: self.notify_change())
    ...
    def remove_filter(self, filter_frame):
        """Remove a filter from the filters chain."""
...
class Effect:
    ...
    def add_effect(self, effects_frame, effect_type="Bitcrusher", params=None):
        """Add a new effect to the effects chain."""
        ...
        # Update UI for the initial effect type
    ...
    def update_effect_ui(self, effect_type, params_frame, params, type_menu):
        """Update the effect UI based on the selected effect type."""
...
class LFO:
    def __init__(self, sample_rate,on_change_callback=None):
        self.sample_rate = sample_rate
        self.lfos = []  # List to store active LFOs
        self.on_change_callback = on_change_callback

    def add_lfo(self, lfos_frame, shape="Sine", frequency=1.0, depth=0.5, target="Frequency"):
        """Add a new LFO to the LFOs chain."""
        lfo_frame = ctk.CTkFrame(lfos_frame)
        ...
        depth_slider = ctk.CTkSlider(lfo_frame, from_=0.0, to=2.0, command=lambda _: self.notify_change())
    ...
    def remove_lfo(self, lfo_frame):
        """Remove an LFO from the LFOs chain."""
